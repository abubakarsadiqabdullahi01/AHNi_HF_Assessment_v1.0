const META = [
  {id:"state",   label:"State",              type:"select", options:["","Borno","Adamawa","Yobe","Taraba","Gombe","Other"], required:true},
  ...
  {id:"period",  label:"Reporting period",   type:"text", ph:"e.g. FY23–FY25", required:true},
...
// Meta field ids that must be filled before a form can be submitted.
export const REQUIRED_META = META.filter((f) => f.required).map((f) => f.id);

// Full field_id -> { section, sectionTitle, question } dictionary for EVERY
// answer field the form can produce. Used to give the Excel export readable
// question text instead of bare field ids. Mirrors the id scheme used by the
// renderer in components/AssessmentForm.jsx.
export function fieldDictionary(){
  const d = {};
  const put = (id, sec, title, question) => { d[id] = { section: sec, sectionTitle: title, question }; };

  for (const sec of SECTIONS) {
    const S = sec.id, TT = sec.title;
    if (sec.type === "qa") {
      sec.items.forEach((raw, i) => {
        const it = normItem(raw);
        const base = `${S}-${i + 1}`, num = `${S}${i + 1}`;
        const q = it.q;
        if (it.kind === B || it.kind === "bin") {
          put(`${base}-r`, S, TT, `${num}. ${q}`);
          put(`${base}-n`, S, TT, `${num}. ${q} — notes`);
        } else if (it.kind === "sub") {
          const n = it.n || 5;
          for (let k = 1; k <= n; k++) put(`${base}-s${k}`, S, TT, `${num}.${k} ${q}`);
        } else if (it.kind === "likert") {
          put(`${base}-lk`, S, TT, `${num}. ${q}`);
          put(`${base}-n`, S, TT, `${num}. ${q} — explanation`);
        } else if (it.kind === "budget") {
          [0, 1, 2].forEach((y) => {
            const yr = `FY${y + 1}`;
            put(`${base}-y${y}-label`, S, TT, `${num}. ${q} — ${yr} label`);
            put(`${base}-state-y${y}`, S, TT, `${num}. ${q} — Total State Budget (${yr})`);
            put(`${base}-health-y${y}`, S, TT, `${num}. ${q} — Total Health Budget (${yr})`);
            put(`${base}-released-y${y}`, S, TT, `${num}. ${q} — Health Budget Released (${yr})`);
            put(`${base}-progareas-y${y}`, S, TT, `${num}. ${q} — Programme areas funded (${yr})`);
            put(`${base}-pctalloc-y${y}`, S, TT, `${num}. ${q} — % allocation to health (${yr})`);
            put(`${base}-pctrel-y${y}`, S, TT, `${num}. ${q} — % health budget released (${yr})`);
          });
        } else {
          put(base, S, TT, `${num}. ${q}`);
        }
      ...
      sec.sources.forEach((src, si) => {
        sec.cols.forEach((c) => put(`${S}-s${si}-${c.key}`, S, TT, `${src} — ${c.label}`));
      });
    } else if (sec.type === "disease") {
      sec.diseases.forEach((dz, di) => {
        dz.items.forEach((item, ii) => {
          put(`${S}-d${di}-${ii}-st`, S, TT, `${dz.name} — ${item} — financing adequacy`);
          put(`${S}-d${di}-${ii}-n`, S, TT, `${dz.name} — ${item} — qualitative note`);
        });
      });
    } else if (sec.type === "checklist") {
      sec.items.forEach((opp, i) => {
        put(`${S}-${i}-s`, S, TT, `${opp} — evidence strength`);
        put(`${S}-${i}-n`, S, TT, `${opp} — specifics`);
      });
    }
  }
  return d;
